...
def settings_index_view(request):
    ...
    context = build_app_context(
        ...
        active_recipients=(
            request.user.form_builder_formrecipient_created.count()
            if hasattr(request.user, "form_builder_formrecipient_created")
            else 0
        ),
        active_schedules=(
            request.user.form_builder_formschedule_created.count()
...
def entry_create_view(request, form_id):
    ...
    if request.method == "POST":
        ...
        if entry_form.is_valid():
            ...
            messages.success(
                request,
                "Entwurf wurde angelegt. Die PDF-Vorschau bleibt waehrend der Bearbeitung sichtbar.",
            )
    ...
    return render_entry_response(
        ...
        build_app_context(
            ...
            live_preview_url="form_builder:entry_pdf_new_live_preview",
        ),
    )
...
def entry_detail_view(request, entry_id):
    ...
    return render(
        ...
        build_app_context(
            ...
            can_edit_entry=can_create_entries(request.user)
            and form_entry.status in EDITABLE_ENTRY_STATUSES,
            can_review_entry=can_review_entries(request.user)
            and form_entry.status == FormEntry.EntryStatus.IN_REVIEW,
            can_queue_entry=can_send_entries(request.user)
...
def entry_pdf_new_live_preview_view(request, form_id):
    ...
    except Exception as exc:  # keep preview endpoint safe in local development
        return HttpResponse(
            f"PDF-Vorschau konnte nicht erzeugt werden: {exc}",
            status=500,
